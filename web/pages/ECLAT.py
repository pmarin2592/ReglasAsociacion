...
class EClAT:
    ...
    def render(self):
        ...
        cols_con_eclat = [
            col for col, algos in results.items() if "eclat" in algos
        ]
        if not cols_con_eclat:
            st.error("No se encontraron resultados de ECLAT.")
            return

        col_sel = st.selectbox(
            "🗂️ Columna de transacción",
            options=cols_con_eclat,
            key="eclat_col_sel",
        )

        rec = results[col_sel]["eclat"]
        ...
        if rules_raw.empty:
            ...
            with st.expander("🔍 Filtrar reglas", expanded=False):
                ...
                min_sup  = fc1.slider("Soporte mínimo",  0.0, 1.0, float(df_display["support"].min()),   0.01, key="ec_fsup")
                min_conf = fc2.slider("Confianza mínima", 0.0, 1.0, float(df_display["confidence"].min()), 0.01, key="ec_fconf")
                min_lift = fc3.slider("Lift mínimo",      1.0, float(df_display["lift"].max()) + 0.1,
                                      float(df_display["lift"].min()), 0.1, key="ec_flift")
        ...
        col_dir = os.path.join(output_dir, col_sel.replace(" ", "_"), "eclat")
